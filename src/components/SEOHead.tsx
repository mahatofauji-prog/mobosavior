...
export default function SEOHead({
  ...
  useEffect(() => {
    ...
    if (serviceData?.seoTitle) {
      ...
      finalTitle = seoSettings?.siteTitle || 'Mobile Repairing Shop in Purulia | MOBO SAVIOR';
    }
    ...
    if (serviceData?.metaDescription) {
      ...
      finalDesc = `Specialized ${categoryData.name} services in Purulia. ${categoryData.description || 'Component-level logic board recovery and screen replacements.'}`;
    } else if (description) {
      finalDesc = description;
    } else {
      finalDesc = seoSettings?.metaDescription || 'MOBO SAVIOR is Purulia\'s specialist mobile phone repair shop operated by Saddam Bhai. Micro-soldering, iPhone screen replacement, CPU reballing, and battery replacements.';
    }
