...
export default function AdminBranches({ onRefreshData }: AdminBranchesProps) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Fetch branches with robust fallback
      let branchData: any[] | null = null;
      ...
      if (!res.error && res.data && res.data.length > 0) {
        ...
        const fallbackRes = await supabase.from('branches').select('*');
        if (fallbackRes.data && fallbackRes.data.length > 0) {
          branchData = [...fallbackRes.data].sort(
            (a: any, b: any) => (a.display_order ?? a.displayOrder ?? 0) - (b.display_order ?? b.displayOrder ?? 0)
          );
        }
      }
