...
import AdminWowSQL from '../components/admin/AdminWowSQL';
...
export default function AdminDashboard({
  ...
  const handleSaveVideoItem = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const active = formData.get('active') === 'true';
      const featured = formData.get('featured') === 'true';
      const fileInput = e.currentTarget.querySelector('input[name="videoFile"]') as HTMLInputElement;
      ...
      <aside className="lg:w-72 bg-white border border-slate-100 rounded-2xl p-4 flex flex-col justify-between shadow-sm flex-shrink-0">
        <div className="space-y-5">
          ...
          <nav className="space-y-4 overflow-y-auto max-h-[calc(100vh-220px)] pr-1">
            ...
            {/* 6. CLOUD & DATABASE */}
            <div>
              <span className="px-2.5 text-[9px] font-extrabold uppercase tracking-wider text-slate-400 block mb-1">6. Cloud & Database</span>
              <div className="space-y-0.5">
                <button
                  onClick={() => setActiveTab('wowsql')}
                  className={`w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-xs font-bold transition-all ${
                    activeTab === 'wowsql' ? 'bg-[#0284C7] text-white shadow-sm ring-2 ring-sky-300' : 'text-slate-700 bg-sky-50/60 hover:bg-sky-100/80 font-black'
                  }`}
                >
                  <span className="flex items-center gap-2">
                    <Database className="w-3.5 h-3.5 text-[#0284C7]" />
                    <span>WOWSQL Database</span>
                  </span>
                  <span className="px-1.5 py-0.2 bg-emerald-100 text-emerald-800 font-extrabold text-[9px] rounded-md">CONNECTED</span>
                </button>
              </div>
            </div>
          </nav>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/55 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full space-y-4 text-xs text-slate-700 relative shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveVideoItem} className="space-y-4">
              <div>
                <label className="block text-slate-500 mb-1">Upload Video File (Firebase Storage)</label>
                <input
                  type="file"
                  name="videoFile"
                  accept="video/*"
              ...
              <div className="relative flex py-2 items-center">
                <div className="flex-grow border-t border-slate-100"></div>
                <span className="flex-shrink mx-3 text-[9px] text-slate-400 font-bold uppercase tracking-wider">or provide public video link</span>
                <div className="flex-grow border-t border-slate-100"></div>
              </div>

              <div>
                <label className="block text-slate-500 mb-1">YouTube URL or Public Video File Link</label>
                <input
                  type="text"
                  name="videoUrl"
                  ...
                  className="w-full px-3.5 py-2 border border-slate-200 rounded-xl font-mono text-[10px]"
                />
                <p className="text-[9px] text-slate-400 mt-1">Recommended: YouTube URLs provide best-in-class loading speeds for local viewers.</p>
              </div>
                ...
                {previewMedia.url.includes('youtube.com') || previewMedia.url.includes('youtu.be') ? (
                  <iframe
                    src={(() => {
                      ...
                      const match = previewMedia.url.match(regExp);
