...
import { getServiceImage, normalizeImageUrl, getCategoryCardImage } from '../utils/serviceImages';
...
const DEFAULT_FALLBACK_SLIDES: SlideItem[] = [
  { id: 'slide-1', imageUrl: '/assets/images/slide_iphone_1788168033319.jpg', title: 'iPhone motherboard repair technician in Purulia', active: true, displayOrder: 1 },
  { id: 'slide-2', imageUrl: '/assets/images/slide_android_1788168049303.jpg', title: 'Android motherboard repair in Purulia', active: true, displayOrder: 2 },
  { id: 'slide-3', imageUrl: '/assets/images/slide_display_1788168074454.jpg', title: 'iPhone display repair service in Purulia', active: true, displayOrder: 3 },
  { id: 'slide-4', imageUrl: '/assets/images/slide_battery_1788168089885.jpg', title: 'Mobile battery replacement service at MOBO SAVIOR Purulia', active: true, displayOrder: 4 },
  { id: 'slide-5', imageUrl: '/assets/images/slide_deadphone_1788168106265.jpg', title: 'Dead mobile phone diagnostic and motherboard repair in Purulia', active: true, displayOrder: 5 },
  { id: 'slide-6', imageUrl: '/assets/images/slide_chargeport_1788168121909.jpg', title: 'Mobile charging port soldering at MOBO SAVIOR Purulia', active: true, displayOrder: 6 },
  { id: 'slide-7', imageUrl: '/assets/images/slide_network_1788168142613.jpg', title: 'Mobile IC repair equipment at MOBO SAVIOR Purulia', active: true, displayOrder: 7 },
  { id: 'slide-8', imageUrl: '/assets/images/slide_cpuic_1788168157704.jpg', title: 'CPU reballing and chip-level mobile repair in Purulia', active: true, displayOrder: 8 },
  { id: 'slide-9', imageUrl: '/assets/images/slide_flip_1788168172190.jpg', title: 'Flip and Fold mobile repair in Purulia', active: true, displayOrder: 9 },
  { id: 'slide-10', imageUrl: '/assets/images/slide_foldable_1788168190438.jpg', title: 'Curved display repair service in Purulia', active: true, displayOrder: 10 }
];
...
export const SPECIALIZED_REPAIR_SECTIONS: SpecializedCategoryConfig[] = [
  {
    ...
    imageAlt: 'iPhone motherboard repair technician in Purulia',
    description: 'Precision servicing for iPhone 11 through iPhone 16 Pro Max. Certified TrueTone calibration, original battery health preservation, Face ID sensor restoration, and dual-layer motherboard micro-soldering in Purulia.',
    highlights: [
  ...
  },
  {
    ...
    imageAlt: 'Android motherboard repair in Purulia',
    description: 'Complete hardware solutions for Samsung Galaxy, OnePlus, Xiaomi, Vivo, Oppo, Realme, and Google Pixel in Purulia. High-spec AMOLED panels, sub-board charging repairs, and network signal restoration.',
    highlights: [
  ...
  },
  {
    ...
    imageAlt: 'Mobile motherboard repair and microsoldering in Purulia',
    description: 'Purulia\'s advanced workstation equipped with 7X-45X stereoscopic microscope, ShortCam II thermal detection, and computerized BGA stencils. Specialized in CPU reballing in Purulia, mobile IC repair, and reviving completely dead phones.',
    highlights: [
  ...
  },
  {
    ...
    imageAlt: 'Flip and Fold mobile repair in Purulia',
    description: 'Expert mechanical and display servicing for Samsung Galaxy Z Fold, Z Flip, and Motorola Razr in Purulia. Crease alignment, hinge debris clearance, flexible OLED ribbon restoration, and ultra-thin glass replacement.',
    highlights: [
  ...
  },
  {
    ...
    imageAlt: 'iPhone display repair service in Purulia',
    description: 'Restore vivid color accuracy and 120Hz touch response. Specializing in factory-grade OLED & AMOLED panels, curved display repair, touch IC replacement, and dust-free OCA glass lamination in Purulia.',
    highlights: [
...
export default function Home({ onNavigate, services, reviews, content, contact, slides = [], categories = [], branches = [], websiteSections }: HomeProps) {
  ...
  // Dynamic Categories: Guarantee all 14 categories from DEFAULT_CATEGORIES plus Supabase overrides
  const mergedCategoriesList: ServiceCategory[] = [...DEFAULT_CATEGORIES];
  if (categories && categories.length > 0) {
    categories.forEach(c => {
      const idx = mergedCategoriesList.findIndex(m => m.id === c.id || m.slug === c.slug || m.name?.toLowerCase() === c.name?.toLowerCase());
      if (idx >= 0) {
        mergedCategoriesList[idx] = {
          ...mergedCategoriesList[idx],
          ...c,
          imageUrl: (c.imageUrl && c.imageUrl.trim() !== '' && !c.imageUrl.includes('placeholder')) ? c.imageUrl : mergedCategoriesList[idx].imageUrl
        };
      } else {
        mergedCategoriesList.push(c);
      }
    });
  }

  const displayCategories = mergedCategoriesList
  ...
  const [homeOpenFaqIndex, setHomeOpenFaqIndex] = useState<number | null>(0);

  const localSeoFaqs = [
    {
      q: 'Where can I get mobile motherboard repair in Purulia?',
      a: 'MOBO SAVIOR provides expert mobile motherboard repair in Purulia at our workstation in Super Market, Hattola More. We handle chip-level micro-soldering, short circuit removal, and logic board recovery for all major mobile brands.'
    },
    {
      q: 'Where can I get iPhone motherboard repair in Purulia?',
      a: 'You can get specialist iPhone motherboard repair at MOBO SAVIOR in Purulia. We diagnose and repair dual-layer iPhone logic boards, dead iPhones, Face ID sensors, Tristar charging chips, and power IC issues using microscopic tools.'
    },
    {
      q: 'Does MOBO SAVIOR repair Android motherboard problems?',
      a: 'Yes, MOBO SAVIOR repairs Android motherboard issues for Samsung, OnePlus, Xiaomi, POCO, Vivo, Oppo, and Realme devices in Purulia, including dead boot, power rail shorts, and CPU reboot loops.'
    },
    {
      q: 'Does MOBO SAVIOR provide CPU reballing in Purulia?',
      a: 'Yes, MOBO SAVIOR offers professional CPU reballing in Purulia for smartphones with CPU/RAM solder disconnection issues, such as POCO X3 Pro, Redmi Note 10 Pro, OnePlus, and Samsung Galaxy series.'
    },
    {
      q: 'Does MOBO SAVIOR provide eMMC/UFS programming?',
      a: 'Yes, MOBO SAVIOR provides advanced eMMC and UFS storage memory programming, firmware flashing, partition recovery, and dead boot recovery services in Purulia.'
    },
    {
      q: 'Where can I get iPhone display repair in Purulia?',
      a: 'MOBO SAVIOR offers high-quality iPhone display repair in Purulia with TrueTone data transfer, touch responsiveness calibration, and original spec OLED panels for iPhone 11 through iPhone 16 Pro Max.'
    },
    {
      q: 'Do you repair curved displays and Flip & Fold mobiles?',
      a: 'Yes, we specialize in curved display replacement and flexible screen repairs for Samsung Galaxy Z Fold & Z Flip, Motorola Razr, and OnePlus edge-to-edge curved OLED devices in Purulia.'
    },
    {
      q: 'Do you provide chip-level mobile IC repair in Purulia?',
      a: 'Yes, MOBO SAVIOR provides chip-level mobile IC repair in Purulia, replacing power management ICs (PMIC), charging ICs, audio ICs, camera driver ICs, and network RF transceivers under stereoscopic microscopes.'
    }
  ];
  ...
  useEffect(() => {
    async function fetchMedia() {
      try {
        ...
        console.warn('Showcase media offline fallback:', err);
      } finally {
    ...
    <div className="flex flex-col pb-16">
      ...
      {isSectionVisible(websiteSections, 'home', 'why_choose_us') && (
        ...
        <div className="text-center max-w-3xl mx-auto space-y-2 sm:space-y-3">
          ...
          {/* Heading directly above social media buttons */}
          <div className="pt-3 sm:pt-4 pb-1 text-center">
            <h3 className="inline-flex items-center justify-center gap-1.5 text-xs sm:text-sm font-extrabold text-pink-500 tracking-tight">
              <Video className="w-4 h-4 text-pink-400 shrink-0" />
              <span>Watch MOBO SAVIOR's Repairing Work Videos</span>
            </h3>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3.5 pt-2 select-none">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3.5 py-2 bg-[#25D366] hover:bg-[#20bd5a] text-white text-xs font-black rounded-xl shadow-[0_4px_12px_rgba(37,211,102,0.25)] hover:shadow-[0_6px_16px_rgba(37,211,102,0.35)] hover:-translate-y-0.5 transition-all active:scale-95 duration-200"
            >
          ...
          {/* 16:9 Offer Card Auto Slider */}
          <ErrorBoundary componentName="Promotional Offers Slider">
      ...
      {isSectionVisible(websiteSections, 'home', 'specialized_services') && (
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-6 lg:gap-8">
          {displayCategories.map((cat) => {
            const catImage = getCategoryCardImage(cat);
                ...
                <div className="h-24 xs:h-36 sm:h-52 bg-slate-950 relative overflow-hidden">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      const fallback = getCategoryCardImage(cat);
      ...
      {/* Local SEO Informational & Keyword Section */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 sm:mt-20">
        <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white rounded-3xl p-6 sm:p-10 border border-slate-700/60 shadow-xl text-left relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-sky-500/10 rounded-full blur-3xl pointer-events-none" />
          <div className="relative z-10 space-y-4 max-w-4xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-sky-500/20 border border-sky-400/30 text-sky-300 rounded-full text-xs font-bold uppercase tracking-wider">
            ...
            <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-white tracking-tight font-sans">
              Professional Mobile Repairing Shop in Purulia
            </h2>
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed font-normal">
              MOBO SAVIOR provides professional mobile repair services in Purulia, West Bengal, including iPhone and Android repair, motherboard-level repair, chip-level diagnostics, CPU reballing, IC repair, display replacement and other advanced mobile repair services.
            </p>
            <div className="pt-2 flex flex-wrap gap-2 text-xs text-sky-200">
              <span className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700">✓ iPhone Motherboard Repair Specialist</span>
              <span className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700">✓ Android CPU Reballing Lab</span>
              <span className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700">✓ eMMC & UFS Memory Programming</span>
              <span className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700">✓ Curved OLED Display Restoration</span>
            </div>
          </div>
        </div>
      </section>

      {/* Local Customer FAQs Accordion Section */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 sm:mt-20">
        <div className="text-center space-y-3 mb-8 sm:mb-12">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-50 text-[#0284C7] text-xs font-black tracking-wider uppercase border border-sky-100">
            <span>Frequently Asked Questions</span>
          </div>
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-tight font-sans">
            Mobile Repair Questions in Purulia
          </h2>
          <p className="text-slate-600 text-sm sm:text-base max-w-2xl mx-auto">
            Clear answers regarding our motherboard micro-soldering, iPhone screen replacements, and chip-level repair lab in Purulia.
          </p>
        </div>

        <div className="max-w-4xl mx-auto space-y-3 text-left">
          {localSeoFaqs.map((faq, idx) => {
            const isOpen = homeOpenFaqIndex === idx;
            return (
              <div
                key={idx}
              ...
              >
                <button
                  onClick={() => setHomeOpenFaqIndex(isOpen ? null : idx)}
                  className="w-full px-5 py-4 sm:px-6 sm:py-5 flex items-center justify-between gap-4 text-left font-bold text-slate-900 hover:text-[#0284C7] transition-colors focus:outline-none"
                >
                  <span className="text-sm sm:text-base font-extrabold pr-2 font-sans">{faq.q}</span>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-transform duration-200 ${isOpen ? 'bg-sky-100 text-[#0284C7] rotate-180' : 'bg-slate-100 text-slate-500'}`}>
