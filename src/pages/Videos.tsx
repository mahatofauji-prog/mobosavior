...
export default function Videos({ onNavigate, contactWhatsapp }: VideosProps) {
  ...
  const getThumbnail = (url: string) => {
    ...
    // For direct uploaded video files, use a clean tech video poster or gradient placeholder
          ...
          {filteredVideos.map((vid) => (
            ...
            >
              ...
              <div 
                ...
                className="relative aspect-video bg-slate-950 flex items-center justify-center overflow-hidden group cursor-pointer"
              >
                <img 
                  src={vid.thumbnailUrl || getThumbnail(vid.videoUrl)} 
                  alt={vid.title}
                  className="absolute inset-0 w-full h-full object-cover group-hover:scale-102 transition-transform duration-500 opacity-80"
                  referrerPolicy="no-referrer"
                  onError={(e) => {
                    // Fallback thumbnail if loading fails
                    (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1581092921461-eab62e97a780?auto=format&fit=crop&q=80&w=600';
                  }}
                />
                
                {/* Visual play button */}
                <div className="absolute inset-0 bg-black/25 flex items-center justify-center group-hover:bg-black/45 transition-colors">
                  <div className="w-14 h-14 bg-[#0284C7] hover:bg-[#0369A1] hover:scale-110 text-white rounded-full flex items-center justify-center shadow-lg transform transition-all">
                    <Play className="w-6 h-6 fill-white ml-1" />
                  </div>
                </div>

                <span className="absolute bottom-4 left-4 px-2.5 py-0.5 bg-black/60 backdrop-blur-sm text-[9px] font-extrabold text-white rounded-lg uppercase tracking-wider">
                  {vid.category}
                </span>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-sm">
            ...
            >
              ...
              <div className="relative aspect-video bg-black w-full">
                {activeVideo.videoUrl.includes('youtube.com') || activeVideo.videoUrl.includes('youtu.be') || activeVideo.videoUrl.includes('embed/') ? (
                  <iframe
                    src={`${getEmbedUrl(activeVideo.videoUrl)}?autoplay=1&mute=0`}
